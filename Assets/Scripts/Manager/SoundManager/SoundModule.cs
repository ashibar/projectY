...
public class SoundModule : MonoBehaviour
{
    ...
    public void BecomeSmaller(float duration)
    {
        ...
        BecomeSmaller_routine(duration, cts.Token);
    }

    public void BecomeLouder(float duration)
    {
        cts = new CancellationTokenSource();
        BecomeLouder_routine(duration, cts.Token);
    }

    private async void BecomeSmaller_routine(float duration, CancellationToken cts)
    {
    ...
    private async void BecomeLouder_routine(float duration, CancellationToken cts)
    {
        float end = Time.time + duration;
        float startValue = audioSource.volume;
